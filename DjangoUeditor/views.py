...
def get_ueditor_settings(request):
    return HttpResponse(json.dumps(USettings.UEditorUploadSettings,
...
def list_files(request):
    ...
    if request.method != "GET":
        return HttpResponse(json.dumps(
            u"{'state:'ERROR'}"), content_type="application/javascript")
    # 取得动作
    ...
    root_path = os.path.join(
        USettings.gSettings.MEDIA_ROOT,
...
def get_output_path(request, path_format, path_format_var):
    ...
    OutputPathFormat = (
        request.GET.get(
            path_format,
            USettings.UEditorSettings["defaultPathFormat"]) %
