...
class RevisionListView(BaseReversionView):
    ...
    def post(self, request, object_id, *args, **kwargs):
        ...
        return TemplateResponse(
            self.request, self.revision_diff_template or self.get_template_list(
                'views/revision_diff.html'),
            context)

    @filter_hook
    def get_media(self):
        return super(RevisionListView, self).get_media() + \
            self.vendor('xadmin.plugin.revision.js', 'xadmin.form.css')
...
class BaseRevisionView(ModelFormAdminView):
    ...
    @filter_hook
    def get_media(self):
        return super(BaseRevisionView, self).get_media() + \
            self.vendor('xadmin.plugin.revision.js')
...
class InlineDiffField(Field):
    ...
    def render(self, form, form_style, context,
        ...
        if not instance.pk:
            return super(InlineDiffField, self).render(
                form, form_style, context)
    ...
    def _hack_inline_formset_initial(self, revision_view, formset):
        ...
        if self.request.method == 'GET' and formset.helper and formset.helper.layout:
            ...
            fake_admin_class = type(str('%s%sFakeAdmin' % (
                self.opts.app_label, self.opts.model_name)), (object, ), {'model': self.model})
            for form in formset.forms:
                instance = form.instance
                if instance.pk:
