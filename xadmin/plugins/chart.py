...
class ChartWidget(ModelBaseWidget):
    ...
    def get_chart_url(self, name, v):
        return self.model_admin_url(
            'chart', name) + "?" + urlencode(self.list_params)
...
class ChartsPlugin(BaseAdminPlugin):
    ...
    def get_chart_url(self, name, v):
        return self.admin_view.model_admin_url(
            'chart', name) + self.admin_view.get_query_string()
...
class ChartsView(ListAdminView):
    ...
    def get(self, request, name):
        ...
        try:
            ...
            if type(xfield) in (models.DateTimeField,
                                models.DateField, models.TimeField):
                option['xaxis'] = {'mode': "time", 'tickLength': 5}
                if isinstance(xfield, models.DateField):
                    option['xaxis']['timeformat'] = "%y/%m/%d"
                elif isinstance(xfield, models.TimeField):
