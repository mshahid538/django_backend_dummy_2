...
class ListAdminView(ModelAdminView):
        ...
        if (self.show_all and self.can_show_all) or not self.multi_page:
            ...
            try:
                ...
                return HttpResponseRedirect(
                    self.request.path + '?' + ERROR_FLAG + '=1')
    ...
    def get_paginator(self):
        return self.paginator_class(
            self.list_queryset, self.list_per_page, 0, True)
    ...
    def get_page_number(self, i):
        if i == DOT:
            ...
            return mark_safe(u'<a href="%s"%s>%d</a> ' % (escape(self.get_query_string(
                {PAGE_VAR: i})), (i == self.paginator.num_pages - 1 and ' class="end"' or ''), i + 1))
