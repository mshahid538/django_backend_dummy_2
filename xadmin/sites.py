        ...
        for model in model_or_iterable:
            if isinstance(model, ModelBase):
                ...
                if model in self._registry_avs:
                    raise AlreadyRegistered(
                        'The admin_view_class %s is already registered' %
                        model.__name__)
                if options:
                    options['__module__'] = __name__
        ...
        for model in model_or_iterable:
            if isinstance(model, ModelBase):
                ...
                if model not in self._registry_avs:
                    raise NotRegistered(
                        'The admin_view_class %s is not registered' %
                        model.__name__)
    ...
    def admin_view(self, view, cacheable=False):
        ...
        def inner(request, *args, **kwargs):
            if not self.has_permission(request) and getattr(
                ...
                return self.create_admin_view(
                    self.login_view)(request, *args, **kwargs)
        ...
        def merge_class(plugin_class):
            if option_classes:
                ...
                for oc in option_classes:
                    ...
                    meta_class = getattr(
                        oc, plugin_class.__name__, getattr(
                            oc, plugin_class.__name__.replace(
                                'Plugin', ''), None))
                    if meta_class:
