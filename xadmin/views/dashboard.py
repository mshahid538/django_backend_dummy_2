...
class BaseWidget(forms.Form):
    ...
    title = forms.CharField(
        label=_('Widget Title'),
        required=False,
        widget=exwidgets.AdminTextInputWidget)
    ...
    def context(self, context):
        ...
        for b in self.q_btns:
            ...
            if 'model' in b:
                ...
                if not self.user.has_perm("%s.view_%s" % (
...
class Dashboard(CommAdminView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return self.template_response(
            'xadmin/views/dashboard.html', self.get_context())
        ...
        if 'id' in request.POST:
            ...
            if request.POST.get('_delete', None) != 'on':
                ...
                try:
                    ...
                    try:
                        portal_pos = UserSettings.objects.get(
                            user=self.user, key="dashboard:%s:pos" %
                            self.get_page_id())
                        pos = [[w for w in col.split(',') if w != str(
...
class ModelDashboard(Dashboard, ModelAdminView):
    ...
    def get(self, request, *args, **kwargs):
        self.widgets = self.get_widgets()
        return self.template_response(self.get_template_list(
            'views/model_dashboard.html'), self.get_context())
