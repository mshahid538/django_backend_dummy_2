    ...
    if token == -1:
        ...
        def _inner_method():
            ...
            if len(fargs) == 1:
                ...
                return fm(func if fargs[1] ==
                          '__' else func(), *args, **kwargs)
...
def filter_hook(func):
    ...
    def method(self, *args, **kwargs):
        ...
        if self.plugins:
            ...
            return filter_chain(filters, len(filters) - 1,
                                _inner_method, *args, **kwargs)
        else:
...
class BaseAdminObject(object):
    ...
    def get_view(self, view_class, option_class=None, *args, **kwargs):
        ...
        return self.admin_site.get_view_class(
            view_class, option_class, **opts)(self.request, *args, **kwargs)
    ...
    def get_admin_url(self, name, *args, **kwargs):
        return reverse('%s:%s' % (self.admin_site.app_name,
    ...
    def get_model_perm(self, model, name):
        return '%s.%s_%s' % (model._meta.app_label, name,
...
class ModelAdminView(CommAdminView):
    ...
    def get_object(self, object_id):
        ...
        """
        # first get base admin view property queryset, return default model
        # queryset
        model = self.model
        try:
    ...
    def get_object_url(self, obj):
        if self.has_change_permission(obj):
            return self.model_admin_url(
                "change", getattr(obj, self.opts.pk.attname))
        elif self.has_view_permission(obj):
            return self.model_admin_url(
                "detail", getattr(obj, self.opts.pk.attname))
        else:
            return None
