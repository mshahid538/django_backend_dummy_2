...
class GridLayoutPlugin(BaseAdminPlugin):
    ...
    def get_layout(self, l):
        item = (isinstance(l, dict)) and l or DEFAULT_LAYOUTS[l]
        return dict({'url': self.admin_view.get_query_string(
            {LAYOUT_VAR: item['key']}), 'selected': False}, **item)
    ...
    def init_request(self, *args, **kwargs):
        ...
        if active:
            layouts = (
                type(
                    self.grid_layouts) in (
                    list,
                    tuple)) and self.grid_layouts or (
                self.grid_layouts,
            )
