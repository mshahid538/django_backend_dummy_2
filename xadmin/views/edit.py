...
class ModelFormAdminView(ModelAdminView):
    ...
    def formfield_for_dbfield(self, db_field, **kwargs):
        ...
        # a field in admin.
        if isinstance(
                db_field, models.ManyToManyField) and not db_field.remote_field.through._meta.auto_created:
            return None
    ...
    def get_field_style(self, db_field, style, **kwargs):
        ...
        if style in (
                'checkbox', 'checkbox-inline') and isinstance(db_field, models.ManyToManyField):
    ...
    def get_field_attrs(self, db_field, **kwargs):
        ...
        if hasattr(db_field, "rel") and db_field.rel:
            ...
            if related_modeladmin and hasattr(
                    related_modeladmin, 'relfield_style'):
                attrs = self.get_field_style(
        ...
        if "_addanother" in request.POST:
            self.message_user(msg +
                              ...
                              (_("You may add another %s below.") %
                               force_text(self.opts.verbose_name)), 'success')
            return request.path
        else:
...
class UpdateAdminView(ModelFormAdminView):
    ...
    def post(self, request, *args, **kwargs):
        if "_saveasnew" in self.request.POST:
            return self.get_model_view(
                CreateAdminView, self.model).post(request)
