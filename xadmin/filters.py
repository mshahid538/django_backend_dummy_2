...
class BaseFilter(object):
    ...
    def get_context(self):
        return {'title': self.title, 'spec': self,
...
class NumberFieldListFilter(FieldFilter):
    ...
    @classmethod
    def test(cls, field, request, params, model, admin_view, field_path):
        return isinstance(field, (models.DecimalField,
...
class RelatedFieldSearchFilter(FieldFilter):
    ...
    def test(cls, field, request, params, model, admin_view, field_path):
        ...
        return related_modeladmin and getattr(
            related_modeladmin, 'relfield_style', None) in ('fk-ajax', 'fk-select')
...
class MultiSelectFieldListFilter(ListFieldFilter):
    ...
    def __init__(self, field, request, params, model, model_admin, field_path,
                 field_order_by=None, field_limit=None, sort_key=None, cache_config=None):
        super(
        ...
        if cache_config is not None and isinstance(cache_config, dict):
        ...
        if field_limit is not None and isinstance(
                field_limit, int) and queryset.count() > field_limit:
            queryset = queryset[:field_limit]

        self.lookup_choices = [
            str(it) for it in queryset.values_list(
    ...
    def choices(self):
        self.lookup_in_val = (
            type(
                self.lookup_in_val) in (
                tuple,
                list)) and self.lookup_in_val or list(
            self.lookup_in_val)
        yield {
